...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         phi_formula=par.index~
                           ...
                           #fixed effect
                           -1+ time1:LH+time1:streamWhite +time2:age_class +time3:LH+time4:LH + time5+time6,
                      
                         p_formula= par.index~
                           #fixed effects
                           -1+ time2+time3+time3:LHsmolt+ time3:LWe_J+ time4 +McN_J:time4 +time5 +time6,
...
obs<-mscjs_fit$dat_TMB$CH*mscjs_fit$dat_TMB$freq
...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         phi_formula=par.index~
                           ...
                           #random
                            diag(0+time1:age_class|mig_year) +  # to trap
                           (0+time2|mig_year) + # (0+time2|mig_year:LH:stream) + #to mcNary
                           (0+time3:age_0|mig_year) +# (0+time3|mig_year:LH:stream) + # to Bonneville
                           (0+time4|mig_year) +diag(0+time4:age_class|mig_year) + #(0+time4|mig_year:LH:stream) +  #to Bonn adults
                         ...
                         p_formula= par.index~
                          ...
                          diag(0+time2:LH|mig_year:stream) + #(0+time2|mig_year:LH:stream) +  # lower trap
                           ...
                           +diag(0+time3|mig_year:age_class:stream) +# (0+time3|mig_year:LH:stream) + # McNary
                           diag(0+time4|mig_year)+
                           diag(0+time4|mig_year:age_class:stream) +# (0+time4|mig_year:LH:stream) +  #Bonneville
...
obs_dat<-mscjs_dat$dat_out %>% 
  ...
  group_by(sea_Year_p,LH,age_class,stream) %>% summarise(across(LWe_J :Tum_A_3, sum)) %>% ungroup()

sim_posterior<-rmvnorm_prec(mu=mscjs_fit$mod$env$last.par.best, 
                            prec=mscjs_fit$fit$SD$jointPrecision, 500, random_seed=1 )

FT_ref_vec<-FT_sim_vec<-numeric(200)
...
# *expected detections*
exp_det<-cbind(mscjs_dat$dat_out %>% select(sea_Year_p:stream), sim$det_1,sim$det_2,sim$det_3) %>% `colnames<-`(colnames(obs_dat %>% select(sea_Year_p  :Tum_A_3))) %>% 
    #sum detection by stream, year, and life history
  group_by(sea_Year_p,LH,age_class,stream) %>% summarise(across(LWe_J :Tum_A_3, sum)) %>% ungroup()
...
sim_obs<-cbind(mscjs_dat$dat_out %>% select(sea_Year_p:stream), sim$sim_det_1,sim$sim_det_2,sim$sim_det_3) %>% `colnames<-`(colnames(obs_dat %>% select(sea_Year_p  :Tum_A_3))) %>% 
    #sum detection by stream, year, and life history
  group_by(sea_Year_p,LH,age_class,stream) %>% summarise(across(LWe_J :Tum_A_3, sum)) %>% ungroup()

#freeman Tukey
FT_ref_vec[i]<- sum((sqrt(obs_dat[,-1:-4])-sqrt(exp_det[,-1:-4]))^2)
FT_sim_vec[i]<- sum((sqrt(sim_obs[,-1:-4])-sqrt(exp_det[,-1:-4]))^2)
if((i/50)%%1==0){gc()}
}
...
summary(FT_ref_vec)
summary(FT_sim_vec)
test<-(cbind(obs_dat %>% select(sea_Year_p:stream),((sim_obs[,-1:-4])-(exp_det[,-1:-4]))))

test %>% group_by(stream,LH) %>% summarise(across(LWe_J :Tum_A_3, sum))
test %>%  select(LWe_J :Tum_A_3) %>%  colSums()


View(test)

colSums( obs_dat %>% select(LWe_J :Tum_A_3) )
head(exp_det)
colSums(sim_obs)


mscjs_fit$par_TMBmscjs_fit$fit$parmscjs_fit$fitmscjs_fit$fit$AIC
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
dat_out<-select(mark_file_CH,sea_Year_p,LH,stream, #grouping variables
                McN_J:Bon_J,Est_J,Bon_A,McJ_A:Tum_A) %>% #sites/occasions to include in model
  #first year with all stream data through last year where data on all three return ages is available (because it is 2020)
  filter(sea_Year_p>=2007&sea_Year_p<=2016) %>%
  #make grouping variables factors
  #mutate_at(vars(sea_Year_p:stream),~as.factor(as.character(.x)))%>% mutate(McN_J=apply(select(.,McN_J,JDD_J),1,max)) %>% select(-JDD_J) %>% 
  #create multistate capture histories
  mutate(ch=select(., c(McN_J:Est_J,Bon_A,McJ_A:Tum_A)) %>%  reduce(paste0)) %>%
