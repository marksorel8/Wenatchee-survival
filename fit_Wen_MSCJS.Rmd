...
site_year_det<-select(mark_file_CH,sea_Year_p,LWe_J:Tum_A) %>% group_by(sea_Year_p) %>% summarise_all(function(x)sum((x>0))) %>% print(n=100)
...
```{r, data_examine}
select(mark_file_CH,sea_Year_p,LH,stream,LWe_J:Tum_A) %>% group_by(LH,stream) %>% summarise_all(function(x)sum((x>0)))%>% rename(released=sea_Year_p)%>% print(n=100)
...
mscjs_dat<- make_dat(mark_file_CH,sites=c( "LWe_J",
  ...
  "Bon_A","McN_A",
  #"PRa_A","RIs_A",
  "Tum_A"),cont_cov=c(),length_bin = 5,doy_bin = 10,inc_unk = FALSE,exc_unk=FALSE) #,"rel_DOY_bin","length_bin"
...
mscjs_dat$n_unk_LH_phi
mscjs_dat$p.pim_unk %>% range()

mscjs_dat$p.design.dat %>% dim()
mscjs_dat$p_pim[[1]] %>% range()
mscjs_dat$Phi.design.dat %>% 
mscjs_dat$dat_out %>% dim()


test_data<-data.frame(y=1,time=factor(rep(letters[10:11],times=9)),timej=(rep(1:0,times=9)),timek=(rep(0:1,times=9)),LH=as.factor(rep(letters[1:3],each=6)),stream=as.factor(rep(letters[4:6],times=6)))

contrasts(test_data$LH)<-contr.sum(3)
contrasts(test_data$stream)<-contr.sum(3)

contrasts(mscjs_dat$Phi.design.dat$stream)<-contr.sum(3)


test<-model.matrix(y~-1+time+timej:LH + timej:stream+timej:stream:LH +timek:LH, data=test_data,contrasts.arg = list(LH=contr.sum(3),stream=contr.sum(3)))
...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         phi_formula=par.index~
                           ...
                           #fixed effect
                           -1+ time1+time1:streamWhite +time2:age_class+time3 +time3:age_class+time4:age_class+time4 +time5+time6,
                      
                         p_formula= par.index~
                           #fixed effects
                           -1+ time2 +time2:streamWhite+time3:age_class+ time4:age_class  +time5  +time6,
                         
                         psi_formula= par.index~
                           #fixed effects
                           -1+tostratum,
                         
                         doFit = TRUE,silent=TRUE,sim_rand = 1,pen=1,hypersd=1,map_hypers=c(FALSE,FALSE)
)
...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         ...
                         phi_formula=par.index~
                            ...
                            ## to Bonneville
                            -1+ time1:LH + pc(0+time1|stream )+ pc(0+time1:stream|LH )+
                            # pc(0+time1:rel_DOY_bin:age_0|one)+
                            # pc(0+time1:sum_flow:LHsummer+time1:sum_air:LHsummer|one) +
                            pc(0+time1:win_flow:age_0+time1:win_air:age_0|one) +
                            # pc(0+time1:spr_dis+time1:spr_air|one) +
                            diag(0+time1|mig_year)+ #diag(0+time1:age_class |mig_year) +
                           
                            ## to McNary
                            time2+ pc(0+time2|age_class) +  #pc(0+time2:LHsummer|one)+
                            # pc(0+time2:RIS_flow_juv+time2:RIS_temp_juv+time2:UC_spill_pct_juv|one) +
                            diag(0+time2 |mig_year)+  #(0+time2:age_class |mig_year)+
                           
                            ##to Bonneville
                            time3 + pc(0+time3|age_class) + # pc(0+time3:LHsummer|one) +
                            diag(0+time3|mig_year)+ #diag(0+time3:age_class |mig_year)+
                            # pc(0+time3:McN_flow+time3:McN_temp+time3:MC_spill_pct_juv|one) +
                           
                            ## SAR to Bonneville adult
                            time4 + pc(0+time4|LH) + (0+time4:age_class |mig_year)+#diag(0+time4|mig_year)+ 
                            pc(0+time4:ersstArc.win+time4:ersstWAcoast.sum|one) +
                            ## to McNary Adult
                            time5 +  # pc(0+time5|stratum ) + #diag(0+time5|mig_year)+                 
                           
                            ## to Tumwater Adults
                            time6 #+ pc(0+time6|stratum ) #+ diag(0+time6|mig_year)
                            ,
                         ...
                         #survival
                            ...
                            -1+ time2:LH + pc(0+time2|stream ) + pc(0+time2:stream|LH ) +  
                           diag(0+time2|mig_year) +
                            
                           ## McNary Juveniles
                            time3 + pc(0+time3|age_class) +
                            diag(0+time3 |mig_year)+ 
                            
                           # Bonneville Juvenile
                            time4 + pc(0+time4|age_class) +  
                            diag(0+time4 |mig_year) + 
                             # pc(0+time4:McN_J:age_0|one) +
                         ...
                         ,
                          #maturation age                        
                            psi_formula= par.index~
                            -1+tostratum + pc(0+tostratum|age_class) + pc(0+tostratum|stream) +
                            (0+tostratum|mig_year)
                           ,
                           
    
                            doFit = FALSE,silent=TRUE,
                            sim_rand =0,REML=FALSE,pen=1,hypersd=1,map_hypers=c(FALSE,FALSE)
)
...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         ...
                         phi_formula=par.index~
                            ...
                            ## to Bonneville
                            -1+ time1+time1:LHsummer+time1:LHsmolt + pc(0+time1|stream )+ pc(0+time1:stream|LH )+
                            # pc(0+time1:rel_DOY_bin:age_0|one)+
                            # pc(0+time1:sum_flow:LHsummer+time1:sum_air:LHsummer|one) +
                            pc(0+time1:win_flow:age_0+time1:win_air:age_0|one) +
                            # pc(0+time1:spr_dis+time1:spr_air|one) +
                            ...
                            ## to McNary
                            time2+ pc(0+time2|LH) +  #pc(0+time2:LHsummer|one)+
                            # pc(0+time2:RIS_flow_juv+time2:RIS_temp_juv+time2:UC_spill_pct_juv|one) +
                            diag(0+time2 |mig_year)+ #(0+time2:age_class |mig_year)+
                           
                            ##to Bonneville
                            time3 + pc(0+time3|LH) + # pc(0+time3:LHsummer|one) +
                            diag(0+time3|mig_year)+#diag(0+time3:age_class |mig_year)+
                            # pc(0+time3:McN_flow+time3:McN_temp+time3:MC_spill_pct_juv|one) +
                           
                            ## SAR to Bonneville adult
                            time4 + pc(0+time4|LH) + (0+time4 |mig_year)+
                            pc(0+time4:ersstArc.win+time4:ersstWAcoast.sum|one) +
                            ## to McNary Adult
                            time5 + #  pc(0+time5|stratum ) + diag(0+time5|mig_year)+                 
                         ...
                         #survival
                            ...
                            ## Lower Wenatchee Trap
                            -1+ time2+time2:LHsummer+time2:LHsmolt  + pc(0+time2|stream ) + 
                           diag(0+time2|mig_year) +
...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         ...
                         phi_formula=par.index~
                            ...
                            ## to Bonneville
                            -1+ time1:LH + time1:streamWhite + pc(0+time1:streamChiwawa|one) +
                           #pc(0+time1:streamChiwawa+time1:streamWhite|one )+ 
                           pc(0+time1:streamChiwawa:LHsummer +
                                time1:streamChiwawa:LHsmolt +
                                time1:streamWhite:LHsummer +
                                time1:streamWhite:LHsmolt|one ) +
                            # pc(0+time1:rel_DOY_bin:age_0|one)+
                            # pc(0+time1:sum_flow:LHsummer+time1:sum_air:LHsummer|one) +
                            pc(0+time1:win_flow:age_0+time1:win_air:age_0|one) +
                            # pc(0+time1:spr_dis+time1:spr_air|one) +
                            ...
                            ## to McNary
                            time2+ pc(0+time2:LHsmolt+time2:LHsummer|one) + 
                            #pc(0+time2:LHsummer|one)+
                            # pc(0+time2:RIS_flow_juv+time2:RIS_temp_juv+time2:UC_spill_pct_juv|one) +
                            diag(0+time2 |mig_year)+  #(0+time2:age_class |mig_year)+
                           
                            ##to Bonneville
                            time3 + pc(0+time3:LHsmolt+time3:LHsummer|one) + # pc(0+time3:LHsummer|one) +
                            diag(0+time3|mig_year) + #diag(0+time3:age_class |mig_year)+
                            # pc(0+time3:McN_flow+time3:McN_temp+time3:MC_spill_pct_juv|one) +
                           
                            ## SAR to Bonneville adult
                            time4 + pc(0+time4:LHsmolt+time4:LHsummer|one) + (0+time4:age_class |mig_year)+#diag(0+time4|mig_year)+ 
                            pc(0+time4:ersstArc.win+time4:ersstWAcoast.sum|one) +
                            ## to McNary Adult
                            time5 +  # pc(0+time5|stratum ) + #diag(0+time5|mig_year)+                 
                         ...
                         #survival
                            ...
                            ## Lower Wenatchee Trap
                            -1+ time2+pc(0+time2:LHsmolt+time2:LHsummer|one) +  
                               pc(0+time2:streamChiwawa+time2:streamWhite|one)+
                               pc(0+time2:streamChiwawa:LHsummer+
                                time2:streamChiwawa:LHsmolt+
                                time2:streamWhite:LHsummer+
                                time2:streamWhite:LHsmolt|one ) +  
                           diag(0+time2|mig_year) +
                            
                           ## McNary Juveniles
                            time3 + pc(0+time3:LHsmolt+time3:LHsummer|one) +
                            diag(0+time3 |mig_year)+ 
                            
                           # Bonneville Juvenile
                            time4 + pc(0+time4:LHsmolt+time4:LHsummer|one) +  
                            diag(0+time4 |mig_year) + 
                           # pc(0+time4:McN_J:age_class|one) +
                         ...
                         ,
                          #maturation age                        
                            psi_formula= par.index~
                            -1+tostratum + pc(0+tostratum:LHsmolt+tostratum:LHsummer|one) +
                            pc(0+tostratum:streamWhite+tostratum:streamChiwawa|one) +
                            (0+tostratum|mig_year)
                           ,
...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         ...
                         phi_formula=par.index~
                            ...
                            ## to Bonneville
                            -1+ time1+time1:LHsmolt+time1:LHsummer + time1:streamWhite + pc(0+time1:streamChiwawa|one) +
                           #pc(0+time1:streamChiwawa+time1:streamWhite|one )+ 
                           pc(0+time1:streamChiwawa:LHsummer +
                                time1:streamChiwawa:LHsmolt +
                                time1:streamWhite:LHsummer +
                            ...
                            ## to McNary
                            time2+ pc(0+time2:LHsmolt+time2:LHsummer+time2:LHUnk|one) + 
                            #pc(0+time2:LHsummer|one)+
                            # pc(0+time2:RIS_flow_juv+time2:RIS_temp_juv+time2:UC_spill_pct_juv|one) +
                            diag(0+time2 |mig_year)+  #(0+time2:age_class |mig_year)+
                           
                            ##to Bonneville
                            time3 + pc(0+time3:LHsmolt+time3:LHsummer+time3:LHUnk|one) + # pc(0+time3:LHsummer|one) +
                            diag(0+time3|mig_year) + #diag(0+time3:age_class |mig_year)+
                            # pc(0+time3:McN_flow+time3:McN_temp+time3:MC_spill_pct_juv|one) +
                           
                            ## SAR to Bonneville adult
                            time4 + pc(0+time4:LHsmolt+time4:LHsummer+time4:LHUnk|one) + (0+time4 |mig_year)+#diag(0+time4|mig_year)+ 
                            pc(0+time4:ersstArc.win+time4:ersstWAcoast.sum|one) +
                            ## to McNary Adult
                            time5 +  # pc(0+time5|stratum ) + #diag(0+time5|mig_year)+                 
                         ...
                         #survival
                           ...
                           ## McNary Juveniles
                            time3 + pc(0+time3:LHsmolt+time3:LHsummer+time3:LHUnk|one) +
                            diag(0+time3 |mig_year)+ 
                            
                           # Bonneville Juvenile
                            time4 + pc(0+time4:LHsmolt+time4:LHsummer+time4:LHUnk|one) +  
                            diag(0+time4 |mig_year) + 
                           # pc(0+time4:McN_J:age_class|one) +
                         ...
                         ,
                          #maturation age                        
                            psi_formula= par.index~
                            -1+tostratum + pc(0+tostratum:LHsmolt+tostratum:LHsummer+tostratum:LHUnk|one) +
                            pc(0+tostratum:streamWhite+tostratum:streamChiwawa+tostratum:streamLWE|one) +
                            (0+tostratum|mig_year)
                           ,
...
# plot(obs_dat_long$value,exp_det_MLE$value) 
...
# dev.new()
ggplot(data= as_tibble(Psi.design.dat), aes(fill=years ,x = mig_year, y =prop)) + geom_bar(stat="identity", width=1)+facet_wrap(~ LH)+ylab("Proportion")+xlab("Smolt year")
...
# dev.new()
ggplot(data= as_tibble(p.design.dat) %>% filter(Time==0),aes(x=mig_year ,y=p_fit,color=LH)) + geom_point(position=position_dodge(width = .5))+geom_linerange(aes(ymin=lcl_p,ymax=ucl_p),position=position_dodge(width = .5))+facet_grid(time~stream,scales="free",labeller =  labeller(time=c(`2`="Lower Wenatchee",`3`="McNary")))+xlab("Smolt year")+ylab("Detection")

ggplot(data= as_tibble(p.design.dat) %>% filter(Time==1),aes(x=mig_year ,y=p_fit,color=LH)) + geom_point(position=position_dodge(width = .5))+geom_linerange(aes(ymin=lcl_p,ymax=ucl_p),position=position_dodge(width = .5))+facet_grid(LWe_J~stream,scales="free",labeller =  labeller(LWe_J=c(`1`="NOT Detected at Lower Wenatchee",`2`="Detected at lwoer wen")))+xlab("Smolt year")+ylab("Detection")+ylim(0,1)

#time 3 (McN_J LH x stream x year)
# dev.new()
ggplot(data= as_tibble(p.design.dat)%>% filter(Time==2),aes(x=mig_year ,y=p_fit,color=LH)) + geom_point(position=position_dodge(width = .5))+geom_linerange(aes(ymin=lcl_p,ymax=ucl_p),position=position_dodge(width = .5))+facet_grid(McN_J~stream,scales="free",labeller =  labeller(McN_J=c(`1`="Bonneville (not detected @ McNary)",`2`="Bonneville (detected @ McNary)")))+ylim(0,1)+xlab("Smolt year")+ylab("Detection")

#time 4-5 (stratum x year)
# dev.new()
ggplot(data= as_tibble(p.design.dat)%>% filter(Time>2),aes(x=mig_year ,y=p_fit,color=stratum)) + geom_point(position=position_dodge(width = .5))+geom_linerange(aes(ymin=lcl_p,ymax=ucl_p),position=position_dodge(width = .5))+facet_grid(~time,scales="free",labeller =  labeller(time=c(`5`="Bonneville (adult)",`6`=" McNary (adult)")))+ylim(0,1)+xlab("Return year")+ylab("Detection")


```

$$logit(\phi^{to~McNary}_{LH,~stream,~year})=\beta^{to~McNary}_{LH}+\beta^{to~McNary}_{stream~White}+\eta^{to~McNary}_{year}+\epsilon^{to~McNary}_{LH,stream,year}\\
\eta_{year}^{to~McNary} \sim N(0,\sigma_{\eta}^2)
$$
...
Look at timing of different life histories at dams
```{r timing}

plot_func<-function(site,mark_file_CH){
...
ggplot(dat=dat , aes(x=x)) +geom_density(fill="grey")+geom_vline(dat=dat , aes(xintercept=med.doy))+
  facet_wrap(~LH,ncol=1)+ggtitle(site)
}


plot_func("LWe_J_doy",mark_file_CH)

plot_func("McN_J_doy",mark_file_CH)

plot_func("JDD_J_doy",mark_file_CH)

plot_func("Bon_J_doy",mark_file_CH)

plot_func("Bon_A_doy",mark_file_CH %>% filter(Bon_A>0) )

plot_func("McN_A_doy",mark_file_CH %>% filter(McN_A>0) )

plot_func("PRa_A_doy",mark_file_CH %>% filter(PRa_A>0) )

plot_func("RIs_A_doy",mark_file_CH %>% filter(RIs_A>0) )

plot_func("Tum_A_doy",mark_file_CH %>% filter(Tum_A>0) )
