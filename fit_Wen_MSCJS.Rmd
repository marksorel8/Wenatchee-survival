...
mscjs_dat<- make_dat(mark_file_CH,sites=c( "LWe_J",
  ...
  "Bon_A","McN_A",
  #"PRa_A","RIs_A",
  "Tum_A"),cont_cov=c(),length_bin = 5,doy_bin = 10,inc_unk = FALSE)#,"rel_DOY_bin","length_bin"
...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         ...
                         doFit = TRUE,silent=TRUE,sim_rand = 1,pen=1,hypersd=1,map_hypers=c(TRUE,TRUE)
)
...
test<-glmmTMB.mod::glmmTMB(formula(par.index~1+ pc(0+time1 |mig_year)), data=mscjs_dat$Phi.design.dat,dispformula = ~0,doFit=FALSE)

test$data.tmb$terms$`0 + time1 | mig_year`
...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         ...
                         +time1:sum_flow:LHsummer+time1:win_air:age_0 + 
                          ...
                          (0+time4 |mig_year) + pc(0+time1:LHfall:streamWhite+ time1:streamWhite:LHsmolt |one_group)+pc(0time1:streamWhite:LHsmolt |one_group)#+
...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         phi_formula=par.index~
                            ...
                            diag(0+time2 |mig_year)+ #diag(0+time2:age_class |mig_year)+
                            ##to Bonneville
                            ...
                            time4 + pc(0+time4|LH) + diag(0+time4|mig_year)+ #diag(0+time4:age_class |mig_year)+
                         ...
                         ,
                           ...
                           ,
                            doFit = TRUE,silent=TRUE,
                            sim_rand =0,REML=FALSE,pen=2,hypersd=1,map_hypers=c(FALSE,FALSE)
)

mscjs_fit$fit

#penalized complexity with length
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         phi_formula=par.index~
                            ## to Bonneville
                            -1+ time1:LH + pc(0+time1|stream )+ pc(0+time1:stream|LH )+
                            pc(0+time1:length_bin:LHsummer|one)+
                            # pc(0+time1:LHfall:length_bin|one)+
                            # pc(0+time1:sum_flow:LHsummer+time1:sum_air:LHsummer|one) +
                            pc(0+time1:win_flow:age_0+time1:win_air:age_0|one) +
                            # pc(0+time1:spr_dis+time1:spr_air|one) +
                            ...
                            # pc(0+time3:McN_flow+time3:McN_temp+time3:MC_spill_pct_juv|one) +
                            ## SAR to Bonneville adult
                            time4 + pc(0+time4|LH) + diag(0+time4|mig_year)+ 
                            pc(0+time4:ersstArc.win+time4:ersstWAcoast.sum|one) +
                            ## to McNary Adult
                            time5 +  # pc(0+time5|stratum ) + #diag(0+time5|mig_year)+                 
                            ## to Tumwater Adults
                           ...
                           diag(0+time2|mig_year) +
                            ...
                            # Bonneville Juvenile
                            time4+ pc(0+time4|age_class) +  
                            diag(0+time4 |mig_year) +# pc(0+time4:McN_J:age_class|one) +
                            # Bonn adult
                            time5 + #pc(0+time5|stratum ) + #diag(0+time5 |mig_year)+
                            #McNary adult
                            time6 #+ pc(0+time6|stratum )# + diag(0+time6 |mig_year)+
...
exp_det_MLE<-cbind(mscjs_dat$releases %>% ungroup() %>% select(LH,stream,sea_Year_p) , sim$det_1,sim$det_2,sim$det_3) %>% `colnames<-`(colnames(obs_dat %>% select(LH  :Tum_A_3))) %>% 
  ...
  filter(!(LH=="Unk" & !name%in%c("McN_J","Bon_J")))
