...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         phi_formula=par.index~
                           ...
                           #random
                            (0+time1:age_0|mig_year) +  # to trap
                         ...
                         p_formula= par.index~
                           ...
                           #random
                          diag(0+time2:LH|mig_year) + #(0+time2|mig_year:LH:stream) +  # lower trap
                           diag(0+time3|mig_year:age_class:stream) +# (0+time3|mig_year:LH:stream) + # McNary
                           diag(0+time4:age_class|mig_year) +# (0+time4|mig_year:LH:stream) +  #Bonneville
...
FT_ref_vec<-FT_sim_vec<-numeric(100)
...
# dev.new()
ggplot(data= as_tibble(p.design.dat) %>% filter(Time<=1),aes(x=mig_year ,y=p_fit,color=LH)) + geom_point(position=position_dodge(width = .5))+geom_linerange(aes(ymin=lcl_p,ymax=ucl_p),position=position_dodge(width = .5))+facet_grid(time~stream,scales="free",labeller =  labeller(time=c(`2`="Lower Wenatchee",`3`="McNary")))+xlab("Smolt year")+ylab("Detection")
