...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         ...
                         p_formula= par.index~
                           #fixed effects
                           -1+ time2+time3+ time4  +time5  +time6,
...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         phi_formula=par.index~
                           ...
                           -1+ time1:LH+ time1:stream+ # time1:streamWhite +#to lower wenatchee trap
                           time2:age_class  + #(0+time1|LH:stream)+ #to mcnary
                           time3:age_class + #time3:LHsmolt +#to Bonneville
                           time4:age_class + #to Bonn adult
                           time5:stratum +   #to McNary
                           time6:stratum +      #to Tumwater
                           #random
                            (0+time1|mig_year) +  # to trap
                           (0+time2|mig_year)+ + # (0+time2|mig_year:LH:stream) + #to mcNary
                           (0+time3|mig_year) +# (0+time3|mig_year:LH:stream) + # to Bonneville
                           (0+time4|mig_year) + #(0+time4|mig_year:LH:stream) +  #to Bonn adults
                            (0+time5|mig_year) #+ #+                              # to Mcanry
                             # (0+time6|mig_year)                               # to Tumwater
                         ,
                         ...
                         p_formula= par.index~
                           #fixed effects
                           -1+ time2:LH+ time2:stream+ time2:LWe_new+# lower trap
                            time3:age_class+     #McNary
                           # time3:LWe_J +               #McNary
                            time4:age_class + #Bonneville
                           # time4:McN_J +   #Bonneville
                           time5:stratum +           # Bonn adult
                           time6:stratum + #:stratum +         #McNary adult
                           #random
                          (0+time2|mig_year) + #(0+time2|mig_year:LH:stream) +  # lower trap
                           (0+time3|mig_year)+ 
                           # +diag(0+time3:age_class|mig_year:stream) +# (0+time3|mig_year:LH:stream) + # McNary
                           (0+time4|mig_year)#+
                           # (0+time4:McN_J|mig_year)#+#Bonneville
                          # (0+time5|mig_year)+
                            # (0+time6|mig_year)
...
obs_dat<-mscjs_dat$dat_out %>% 
  ...
  group_by(LH,age_class,stream,sea_Year_p) %>% summarise(across(LWe_J :Tum_A_3, sum)) %>% ungroup()
...
exp_det_MLE<-cbind(obs_dat %>% select(LH:sea_Year_p) , sim$det_1,sim$det_2,sim$det_3) %>% `colnames<-`(colnames(obs_dat %>% select(LH  :Tum_A_3))) %>% 
    #sum detection by stream, year, and life history
  group_by(LH,age_class,stream,sea_Year_p) %>% summarise(across(LWe_J :Tum_A_3, sum)) %>% ungroup() %>% pivot_longer(LWe_J :Tum_A_3) 
...
out_group<-recalculateResiduals(dharm_sim,group=paste(obs_dat_long$name,obs_dat_long$LH))
