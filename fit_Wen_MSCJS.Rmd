...
Juvenile life history diversity is thought to increase population stability, but the mechanisms underlying that stability are rarely understood because it is difficult to track the performance of individuals that expressed different juvenile life histories throughout their lives. We analyzed a rare long-term mark-recapture data set of Chinook salmon that were observed to express different juvenile life history tactics with regard to emigration timing from natal stream. Juvenile salmon either emigrated from their natal stream in their first year of life and spent months in downstream rearing habitat prior to migrating to the ocean or remained in their natal stream entirely until migrating to the ocean. All fish migrated to the ocean in their second spring of life. Earlier emigrants from the natal stream entered the migration corridor about 20 days earlier on average and migrated somewhat slower and had lower downstream-survival rates than later emigrants in the upper migration corridor. Earlier emigrants from the natal stream returned from the ocean at higher rates and somewhat younger ages. Overwinter survival of early emigrants downstream of natal areas prior to seaward emigration was found to be negatively related to winter air temperature, and as in other recent studies, marine survival was positively related to coastal upwelling in spring and negatively related to sea surface temperature in summer. Understanding differences in survival and maturation age of animals that exhibit different juvenile life history strategies leads to a more holistic conceptualization of the factors governing population productivity and stability, enabling more mechanistic simulation of the affects of management and climate change on populations.   
...
# save list of all LOOICs
 # save(Loo_pen2,file=here("Loo_pen8.Rdata"))
...
#Loo_pen8.Rdata = LH intercepts year and random effects (summer and smolt) 
...
load(file=here("Loo_pen6.Rdata"))
Looic<-unlist(lapply(Loo_pen2,function(x){x$estimates[3,1]}))
plot(c(5:8,10:15),Looic,xlab=expression(lambda),ylab="LOOIC",pch=19)
```
...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         phi_formula=par.index~
                           ...
                           #fixed effect
                           -1+ time1:LH+time1:streamWhite:LH+ +time2+time2:age_class+time3+time3:age_class +time4:LH+time4 +time5+time6,
                      
                         p_formula= par.index~
                           #fixed effects
                           -1+ time2:LH+time3+time3:age_class+time4+time4:age_class+  +time5  +time6+time3:age_class:LWe_J+time4:McN_J:age_class, 
                         
                         psi_formula= par.index~
                           #fixed effects
                           -1+tostratum+tostratum:age_class,
                         
                         doFit = TRUE,silent=TRUE,sim_rand = 0,pen=1.5,hypersd=1,map_hypers=c(FALSE,FALSE)
)
