...
mscjs_dat<- make_dat(mark_file_CH,sites=c( "LWe_J",
  ...
  "Bon_A","McN_A",
  #"PRa_A","RIs_A",
  "Tum_A"),cont_cov=c(),length_bin = 5,doy_bin = 10)#,"rel_DOY_bin","length_bin"
...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         phi_formula=par.index~
                           ...
                           #fixed effect
                           -1+ time1:LH+time1:streamWhite +time2:age_class+time3 +time3:age_class+time4:age_class+time4 +time5+time6,
                      
                         p_formula= par.index~
                           #fixed effects
                           -1+ time2:LH +time2:streamWhite+time3:age_class+ time4:age_class  +time5  +time6,
                         
                         psi_formula= par.index~
                           #fixed effects
                           -1+tostratum,
                         
                         doFit = TRUE,silent=TRUE,sim_rand = 1
)

mscjs_fit2<-fit_wen_mscjs(x=mscjs_dat,
                         phi_formula=par.index~
                         ...
                         p_formula= par.index~
                           #fixed effects
                           -1+ time2:LH +time2:White+time3:age_class+ time4:age_class  +time5  +time6,
                         
                         psi_formula= par.index~
                           #fixed effects
                           -1+tostratum,
                         
                         doFit = TRUE,silent=TRUE,sim_rand = 0
)



print_out(mscjs_fit2)
test<-mscjs_fit$mod$simulate()
obs<-mscjs_fit$dat_TMB$CH
obs[obs>0]<-1
obs<-obs*mscjs_fit$dat_TMB$freq
colSums(obs)
colSums(test$sim_det_1)
colSums(test$sim_det_2)
colSums(test$sim_det_3)
colSums(test$det_1)
colSums(test$det_2)
colSums(test$det_3)
...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         phi_formula=par.index~
                           ...
                           #fixed effect
                           -1+ time1:LH:stream+ time1:length_bin:LHsummer +
                           time2:LH+ time2:stream+#time2:length_bin:LHsummer+
                           time3:LH+ time3:stream+# time3:length_bin:LHsummer+
                           time4:stratum+time5:stratum+
                           #random
                           (0+time1|mig_year)+ (0+time1|mig_year:LH:stream)+(0+time1:length_bin:LHsummer|mig_year)+
                           (0+time2|mig_year)+ (0+time2|mig_year:LH)+(0+time2|mig_year:stream)+
                           (0+time3|mig_year)+ (0+time3|mig_year:LH)+(0+time3|mig_year:stream)+
                           (0+time4|mig_year)+
                           (0+time5|mig_year)
                         ...
                         p_formula= par.index~
                           #fixed effects
                           -1+ time2:LH+time2:stream+
                           time3:LH+ time3:stream+ #time3:length_bin:LHsummer+
                           time3:LH:McN_J+
                           time4:stratum+time5:stratum+
                           #random
                          (0+time2|mig_year)+ (0+time2|mig_year:LH:stream)+
                          (0+time3|mig_year)+ (0+time3|mig_year:LH)
                           (0+time3:McN_J|mig_year)+ (0+time3:McN_J|mig_year:LH)+
                          (0+time4|mig_year)+
                           (0+time5|mig_year)
                         ,
...
mscjs_fit$p.design.glmmTMB$data.tmb$terms
...
# 
#                            #fixed effect
#                            -1+ time1:LH+ time1:streamWhite+ #(0+time1|LH:stream)+ #time1:length_bin:LHsummer+
#                            time2+ time2:LHsmolt + #(0+time2|LH:stream)+
#                            time3+ time3:LHsmolt+ #(0+time3|LH:stream)+
#                            time4 +
#                            time5+time5:stratum3+
#                            #random
#                            (0+time1|mig_year)+ (0+time1|mig_year:LH:stream)+# (0+time1:length_bin:LHsummer|mig_year)+
#                            # (0+time2|mig_year)+ (0+time2|mig_year:LH:stream)+
#                            (0+time3|mig_year)+ (0+time3|mig_year:LH:stream)+
#                            (0+time4|mig_year)#+
#                             # (0+time5|mig_year)
# 
#                          ,
...
#                          p_formula= par.index~
#                            #fixed effects
#                            -1+ time2 + time2:LHsmolt+  #(0+time2|LH:stream)+ #time2:length_bin:LHsummer +
#                            time3+ #(0+time3|LH:stream)+
#                            time3:McN_J+# (0+time3:McN_J|LH)+
#                             time4:stratum +
#                            time5+#:stratum +
#                            #random
#                           (0+time2|mig_year)+ (0+time2|mig_year:LH:stream)+#(0+time2:length_bin:LHsummer|mig_year)+
#                           (0+time3|mig_year)+ (0+time3|mig_year:LH:stream)+
#                            (0+time3:McN_J|mig_year) #+ #(0+time3:McN_J|mig_year:LH)+
#                          # (0+time4|mig_year)+
#                           # (0+time5|mig_year)
...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         phi_formula=par.index~
                           ...
                           #fixed effect
                           -1+ time1:LH+ time1:stream+ # time1:streamWhite +#to lower wenatchee trap
                           time2:age_class  + #(0+time1|LH:stream)+ #to mcnary
                           time3:age_class + time3:streamNason+#time3:LHsmolt +#to Bonneville
                           time4:age_class + time4:streamChiwawa+#to Bonn adult
                           time5:age_class +              #to McNary
                           time6 + time6:stratum3 +      #to Tumwater
                           #random
                            diag(0+time1:age_0|mig_year) +  # to trap
                           (0+time2|mig_year) + # (0+time2|mig_year:LH:stream) + #to mcNary
                           (0+time3:age_0|mig_year) +# (0+time3|mig_year:LH:stream) + # to Bonneville
                           (0+time4|mig_year) +diag(0+time4|mig_year:age_class) + #(0+time4|mig_year:LH:stream) +  #to Bonn adults
                            (0+time5|mig_year) #+                              # to Mcanry
                             # (0+time6|mig_year)                               # to Tumwater
                         ,
                      
                         p_formula= par.index~
                           #fixed effects
                           -1+ time2:LH+ time2:streamWhite + time2:LWe_new+# lower trap
                            time3:age_class+     #McNary
                           # time3:LWe_J:age_0 +               #McNary
                            time4:age_class + time4:streamChiwawa +  #Bonneville
                           # time4:McN_J +   #Bonneville
                           time5:stratum +           # Bonn adult
                           time6 + #:stratum +         #McNary adult
                           #random
                          diag(0+time2:LH|mig_year:stream) + #(0+time2|mig_year:LH:stream) +  # lower trap
                           diag(0+time3|mig_year)+ 
                           +diag(0+time3|mig_year:stream:age_class) +# (0+time3|mig_year:LH:stream) + # McNary
                           diag(0+time4|mig_year)+
                           diag(0+time4|mig_year:stream:age_class) #+# (0+time4|mig_year:LH:stream) +  #Bonneville
                           # diag(0+time4:McN_J|mig_year)#Bonneville
                         ,
                         
                         psi_formula= par.index~
                           #fixed effects
                           -1+tostratum+tostratum:LHsmolt+tostratum:streamNason+
                           #random
                            (0+tostratum|mig_year)
                         ,
...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         phi_formula=par.index~
                           ...
                           #fixed effect
                           -1+ time1+time1:LHsmolt+time1:LHsummer+ time1:streamWhite+ 
                           time2 + time2:LHsmolt  + time2:streamNason+ #to mcnary
                           time3 + #to Bonneville
                           time4 + time4:LHsmolt + time4:LHsummer+#to Bonn adult
                           time5 +   #to McNary
                           time6 + time6:stratum3 +    #to Tumwater
                           #random
                           (0+time1:LHfall+time1:LHsummer |mig_year) +
                           (0+time2 |mig_year) +
                           #diag(0+time1:streamNason|mig_year)+
                           # (0+time1+time2|mig_year)+
                           (time4+0|mig_year)  +  diag(0+time4:LHsmolt|mig_year) 

                         ,
                      
                         p_formula= par.index~
                           #fixed effects
                           -1+ time2+time2:LHsmolt+time2:LHsummer + time2:streamChiwawa+time2:streamWhite+ time2:LWe_new+# lower trap
                            time3+     #McNary
                           # time3:LWe_J +               #McNary
                            time4+ #Bonneville
                           ...
                           time6 + #:stratum +         #McNary adult
                           #random
                          (0+time2|mig_year) + diag(0+time2:LHfall+time2:LHsummer|mig_year) + # lower trap
                           (0+time3+time4|mig_year)  # McNary
                           # (0+time4|mig_year) #Bonneville
                          # (0+time5|mig_year)+
                            # (0+time6|mig_year)
                         ...
                         psi_formula= par.index~
                           #fixed effects
                           -1+tostratum+tostratum:LHsmolt+tostratum:streamChiwawa+
                           #random
                             (0+tostratum|mig_year)
                         ,
                         
                         doFit = TRUE,silent=TRUE,
                         sim_rand = 1,REML=FALSE
)
...
mscjs_fit<-fit_wen_mscjs(x=mscjs_dat,
                         phi_formula=par.index~
                           ...
                           #fixed effect
                           -1+ time1 + time1:LHsmolt + time1:LHsummer+ time1:streamWhite+ time1:streamNason+
                           time2 + time2:LHsmolt+   #to mcnary
                           time3 + #to Bonneville
                           time4 + time4:LHsmolt + time4:LHsummer+ #to Bonn adult
                           time5 +   #to McNary
                           time6 +    #to Tumwater
                           #random
                           # cs(0+time1:LH| mig_year) +
                           (0+time1|mig_year) +
                            (0+time2|mig_year) +
                            (time4+0|mig_year)
                           # (0|mig_year) +
                         ...
                         p_formula= par.index~
                           #fixed effects
                           -1+ time2+time2:LHsmolt +# lower trap
                            time3 +  #McNary
                            time4 +  #Bonneville
                            time4:McN_J +   #Bonneville
                           time5 +         # Bonn adult
                           time6 + #:stratum +         #McNary adult
                           #random
                          cs(0+time2:LH|mig_year)+# + #diag(0+time2:LHfall+time2:LHsummer|mig_year) + # lower trap
                            diag(0+time3+time4|mig_year)  # McNary
                           # (0+time4|mig_year) #Bonneville
                          # (0+time5|mig_year)+
                            # (0+time6|mig_year)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
dat_out<-select(mark_file_CH,sea_Year_p,LH,stream, #grouping variables
                McN_J:Bon_J,Est_J,Bon_A,McN_A:Tum_A) %>% #sites/occasions to include in model
  #first year with all stream data through last year where data on all three return ages is available (because it is 2020)
  filter(sea_Year_p>=2007&sea_Year_p<=2016) %>%
  #make grouping variables factors
  #mutate_at(vars(sea_Year_p:stream),~as.factor(as.character(.x)))%>% mutate(McN_J=apply(select(.,McN_J,JDD_J),1,max)) %>% select(-JDD_J) %>% 
  #create multistate capture histories
  mutate(ch=select(., c(McN_J:Est_J,Bon_A,McN_A:Tum_A)) %>%  reduce(paste0)) %>%
  #mutate(ch=select(., McN_J,Bon_J,Est_J) %>% reduce(paste0)) %>%
  mutate(ch=paste0("1",ch)) 

library(RMark)
wenatchee.processed=process.data(dat_out,model="CJS")
...
test<-RMark::mark(wenatchee.processed,wenatchee.ddl,
                  model.parameters=list(Phi=Phi.timexstratum.tag,p= p.timexstratum.tag),run=TRUE)
